...
from argsupport.gooey.gui.util.freeze import getResourcePath
...
def _get_font_weight(kwargs):
    ...
    ```
    from argsupport.gooey import constants
...
def parse_events(events: List[str]) -> List[str]:
    ...
    if unknown_events:
        raise ValueError(
            ...
            f'Consider using the `Events` object: `from argsupport.gooey import Events`')
    else:
        return events
